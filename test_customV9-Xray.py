...
# Argument Parser
parser = argparse.ArgumentParser(description='Super Resolution Processing with Five Models (PSNR/SSIM) - V9')
...
parser.add_argument('--model2_name', default='/CT_SRGAN_DHT__netG_epoch_4_92.pth', type=str, help='second generator model')
...
parser.add_argument('--wgan_name', default='WGAN_epoch_080.pth', type=str, help='W-GAN model weights')
...
print("=" * 100)
print("Starting Super-Resolution Comparison with Five Models (PSNR/SSIM Only) - V9")
print("W-GAN metrics calculated on grayscale (single-channel)")
print("=" * 100)
...
for image_name in os.listdir(TEST_FOLDER):
    ...
    # Calculate metrics on grayscale (single channel) for W-GAN
    hr_gray = hr_image.convert('L')
    metrics_wgan = calculate_metrics(hr_gray, sr_image_wgan_gray)
    ...
    # For comparison grid, convert to RGB for display
    sr_image_wgan_rgb = sr_image_wgan_gray.convert('RGB')
    comparison_images[WGAN_LABEL] = {
        'image': sr_image_wgan_rgb,
...
csv_path = os.path.join(OUTPUT_FOLDER, "detailed_results_v9.csv")
if results:
...
summary_csv_path = os.path.join(OUTPUT_FOLDER, "summary_results_v9.csv")
...
# Example command:
# python test_customV9.py --test_folder "/home/dst/Desktop/GAN/SRGAN/Test_set" --output_folder "/home/dst/Desktop/GAN/SRGAN/output"
