...
# Argument Parser
parser = argparse.ArgumentParser(description='Super Resolution Processing with Three Models + Comprehensive Metrics')
...
if TEST_MODE:
    model_srcnn.cuda()
    checkpoint = torch.load(SRCNN_NAME)
else:
    checkpoint = torch.load(SRCNN_NAME, map_location=torch.device('cpu'))

# Load SRCNN weights (handle different checkpoint formats)
if 'state_dict' in checkpoint:
    model_srcnn.load_state_dict(checkpoint['state_dict'])
else:
    model_srcnn.load_state_dict(checkpoint)

# Calculate FLOPs for all models (one-time calculation)
dummy_input_rgb = torch.randn(1, 3, 64, 64)  # RGB input for SRGAN models
dummy_input_y = torch.randn(1, 1, 64, 64)    # Y channel input for SRCNN
...
def add_label_to_image(image, label, metrics_text=""):
    ...
    try:
        ...
        font_metrics = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf", 11)
    except:
...
def create_comparison_grid(images_dict, image_name):
    ...
    method_order = ['GT', 'LR', MODEL1_LABEL, MODEL2_LABEL, SRCNN_LABEL, 'Bicubic', 'Lanczos']
...
print("=" * 110)
print("Starting Super-Resolution Comparison with Three Models + Comprehensive Metrics")
print("=" * 110)
...
for image_name in os.listdir(TEST_FOLDER):
    ...
    # Merge with CbCr from bicubic upscaled image
...
summary_results = []
all_methods = [MODEL1_LABEL, MODEL2_LABEL, SRCNN_LABEL, 'Nearest', 'Bilinear', 'Bicubic', 'Lanczos']
...
csv_path = os.path.join(OUTPUT_FOLDER, "detailed_results_v6.csv")
if results:
...
summary_csv_path = os.path.join(OUTPUT_FOLDER, "summary_results_v6.csv")
...
print(f"  - Models compared: {MODEL1_LABEL}, {MODEL2_LABEL}, {SRCNN_LABEL} + 4 interpolation methods")
print("=" * 110)

# Example command:
# python test_customV6.py --test_folder "/home/dst/Desktop/GAN/SRGAN/Test_set" --output_folder "/home/dst/Desktop/GAN/SRGAN/output"
